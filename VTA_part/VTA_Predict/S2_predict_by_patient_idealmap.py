...
from S1_Func_idealmap_and_similarity_patient import *

if __name__ == '__main__':
    set_environ()
    work_dir = '/path/to/your/workspace/DBS_Open_Project/VTA_FC_predict/predict_func_patient_N14'
    ...
    for sub in tqdm.tqdm(n_subjects):
        ...
        gts[start_index:(start_index + len(test_data))] = test_data['change_rate_1m'].values
        subs.append(test_data['Subject'].values[0])

        sub_name = test_data['Subject'].values[0]
        # 2. Weight map
        weight_map(train_data, work_dir, str(sub))
        # 3. R map
        R_map(train_data, work_dir, str(sub))
        # 4. Ideal map
        ideal_map(work_dir, str(sub))
    ...
    plot_info['sub'] = subs
    plot_info.to_csv(f"{work_dir}/patient_idealmap_predict_N14.csv", index=False)
